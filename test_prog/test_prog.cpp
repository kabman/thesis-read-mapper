...
// parse command line
	...
	mapper_subst_scores sub_scores;
	...
	mapper_copy_subst_scores(&sub_scores);
	...
	mapper_gpu_storage_t *gpu_storage_arr =(mapper_gpu_storage_t*)calloc(n_threads, sizeof(mapper_gpu_storage_t));
	int z;
	for (z = 0; z < n_threads; z++) {//allocate GPU memory for all threads
		mapper_gpu_mem_alloc(&(gpu_storage_arr[z]), MAX_SEQ_LEN*GPU_BATCH_SIZE, MAX_SEQ_LEN*GPU_BATCH_SIZE, GPU_BATCH_SIZE, LOCAL, WITH_START);
	}
	#pragma omp parallel
	{
	...
	if (n_seqs > 0) {
		while (seqs_done < n_seqs) {
			...
			//mapper_gpu_storage *gpu_storage = NULL;
			...
			//------------------call the mapper alignment function---------------------
			if (al_type.compare("local") == 0){
				if(start_pos){
					mapper_aln(&(gpu_storage_arr[omp_get_thread_num()]), query_batch.data(), query_batch_offsets.data(), query_batch_lens.data(), target_batch.data(), target_batch_offsets.data(), target_batch_lens.data(), query_batch_bytes, target_batch_bytes, actual_n_seqs, scores.data(), query_batch_start.data(), target_batch_start.data(), query_batch_end.data(), target_batch_end.data(), LOCAL, WITH_START);

				}
				else {
					mapper_aln(&(gpu_storage_arr[omp_get_thread_num()]), query_batch.data(), query_batch_offsets.data(), query_batch_lens.data(), target_batch.data(), target_batch_offsets.data(), target_batch_lens.data(), query_batch_bytes, target_batch_bytes, actual_n_seqs, scores.data(), NULL, NULL, query_batch_end.data(), target_batch_end.data(), LOCAL, WITHOUT_START);
				}
			}
			else if (al_type.compare("semi_global") == 0) {
				if (start_pos) {
					mapper_aln(&(gpu_storage_arr[omp_get_thread_num()]), query_batch.data(), query_batch_offsets.data(), query_batch_lens.data(), target_batch.data(), target_batch_offsets.data(), target_batch_lens.data(), query_batch_bytes, target_batch_bytes, actual_n_seqs, scores.data(), NULL, target_batch_start.data(), NULL, target_batch_end.data(), SEMI_GLOBAL, WITH_START);

				} else {
					mapper_aln(&(gpu_storage_arr[omp_get_thread_num()]), query_batch.data(), query_batch_offsets.data(), query_batch_lens.data(), target_batch.data(), target_batch_offsets.data(), target_batch_lens.data(), query_batch_bytes, target_batch_bytes, actual_n_seqs, scores.data(), NULL, NULL, NULL, target_batch_end.data(), SEMI_GLOBAL, WITHOUT_START);

				}
			} else {
				mapper_aln(&(gpu_storage_arr[omp_get_thread_num()]), query_batch.data(), query_batch_offsets.data(), query_batch_lens.data(), target_batch.data(), target_batch_offsets.data(), target_batch_lens.data(), query_batch_bytes, target_batch_bytes, actual_n_seqs, scores.data(), NULL, NULL, NULL, NULL, GLOBAL, WITHOUT_START);
...
#pragma omp critical
	...
	for (z = 0; z < n_threads; z++) {//release GPU memory
		mapper_gpu_mem_free(&gpu_storage_arr[z]);;
	}
